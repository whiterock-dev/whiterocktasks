    ...
    if (after.is_recurring_master) {
      try {
        ...
        if (!activeChildrenSnap.empty) {
          ...
          await batch.commit();
          logger.info(`Propagated SOP updates from master task ${taskId} to ${activeChildrenSnap.size} active child tasks.`);
        }
      } catch (err) {
        logger.error(`Failed to propagate SOP updates for master task ${event.data.after.id}:`, err);
      }
    }
